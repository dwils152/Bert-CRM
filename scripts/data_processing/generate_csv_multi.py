...
class CsvGenerator():
    ...
    def process_sequences(self, records):
        """
        Process a batch of sequences and return the formatted output.
        """
        results = []
        for record in records:
            ...
            results.append(f'{record.seq}: {labels_str}| {coords.chrom}-{coords.start}-{coords.end}\n')
        return results

    def generate_csv(self, output: Path, num_processes=None):
        seqs = list(self._parse_fasta())
        num_processes = num_processes or multiprocessing.cpu_count()

        with Pool(processes=num_processes) as pool:
            chunks = [seqs[i:i + len(seqs) // num_processes + 1] for i in range(0, len(seqs), len(seqs) // num_processes + 1)]
            results = pool.map(self.process_sequences, chunks)
...
if __name__ == "__main__":
    ...
    p.sort_stats('cumulative').print_stats(10)  # Sort by cumulative time and print the top 10
