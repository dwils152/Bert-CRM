...
/*
  ...
  Assumes each unique ID has exactly 2047 lines. Creates a memory-mapped file of shape (num_ids, 2047).
  ...
  Steps:
   ...
   2) Check line_count == num_ids * 2047. If mismatch, abort.
   3) Create (truncate) the memmap file to hold num_ids * 2047 * sizeof(int32_t).
   4) PASS 2:
   ...
   5) Verify each row got exactly 2047 labels.
  ...
  NOTE: 
   ...
   - For billions of lines, expect a long runtime unless you have high I/O bandwidth. 
   - Each pass reads ~N lines, so total ~2N line parses.
*/

static const size_t LABELS_PER_ID = 2047;
...
int main(int argc, char** argv) {
    ...
    // Validate each ID should have exactly 2047 lines
    if (line_count != (uint64_t)LABELS_PER_ID * num_ids) {
        std::cerr << "Error: line_count (" << line_count 
                  << ") != 2047 * num_ids (" << (LABELS_PER_ID * num_ids) << ")\n"
                  << "Not every ID has exactly 2047 lines, or file is malformed.\n";
        return 1;
    }
    ...
    close(fd); // We can close fd after a successful mmap
    ...
    // If you expect >65535 lines per ID, use uint32_t. But here we have exactly 2047 lines/ID.
    ...
    while (true) {
        ...
        if (col_idx >= LABELS_PER_ID) {
            std::cerr << "Error: ID '" << bed_id 
                      << "' has more than 2047 lines.\n";
            munmap(map_ptr, array_size);
            return 1;
        }

        // row-major offset = row_idx * 2047 + col_idx
