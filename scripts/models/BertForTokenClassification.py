...
class BertForTokenClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        encoder_output, pooling_output = self.bert(
        ...
        # return self.classifier(encoder_output), attention_probs
    ...
    def train_model(self, train_loader, val_loader, optimizer, device, rank):
        ...
        for batch in tqdm_wrapper(train_loader, rank, desc="Training"):
            input_ids, attention_mask, labels, coords = [
            ...
            '''logits, attn_matrix  = self.forward(input_ids, attention_mask, labels)

            last_layer = attn_matrix[-1]
            heads = torch.stack(last_layer, dim=0) # 12 heads x batch_size x num_tokens x num_tokens
            #swap the batch_size and heads dimensions
            heads = heads.permute(1, 0, 2, 3).cpu().detach().numpy()
            print(type(heads))
            coords_array= np.array([coord[1] for coord in coords]) # chr-1000-2000
            np.savez(f'coords_{i}.npz', heads=heads)'''

            # print(type(attn_matrix))
            # print(len(attn_matrix))
            # print(attn_matrix[0])
