...
class SupervisedDataset(Dataset):
        ...
        with open(self.data_path, "r") as fin:
            for line in fin:
                ...
                if len(parts) == 3:  # Ensure each line has the correct format
                    ...
                    additional_infos.append(additional_info.strip())
                else:
    ...
    def __getitem__(self, i):
        ...
        # Alter the attention mask so that input_ids of 1 and 2 are masked
        attention_mask = torch.where(
            input_ids == 1, torch.tensor(0), attention_mask)
        attention_mask = torch.where(
            input_ids == 2, torch.tensor(0), attention_mask)
...
if __name__ == "__main__":
    ...
    #change torch settings to print entire tensr    
    torch.set_printoptions(profile="full")
    print("Testing SupervisedDataset")
    datapath = 'chunk_99.fa.csv'
    dataset = SupervisedDataset(datapath, 1024)
    first_ex = dataset[-1]
    first_ex_input_ids, first_ex_attention_mask, first_ex_labels, first_ex_coords = first_ex
    print(first_ex_input_ids)
    print(first_ex_attention_mask)
