...
def main(args):
    ...
    trainloader = DataLoader(Subset(dataset, train_indices), batch_size=128,
                             sampler=train_sampler, shuffle=False, num_workers=8, collate_fn=collate_fn)
    valloader = DataLoader(Subset(dataset, val_indices), batch_size=128,
                           sampler=val_sampler, shuffle=False, num_workers=8, collate_fn=collate_fn)
    testloader = DataLoader(Subset(dataset, test_indices), batch_size=128,
                            sampler=test_sampler, shuffle=False, num_workers=8, collate_fn=collate_fn)
