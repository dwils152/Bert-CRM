...
workflow PREPROCESS_SINGLE_NT {
    ...
    main:
        ...
        generate_labels(tokenize_genome.out, crm)
...
process mask_genome {
    ...
    label "Orion"
    publishDir "${params.publish_dir}/Single-NT_Preprocess/${organism}", mode: 'copy' 
    input:
...
process tokenize_genome {
    ...
    input:
        tuple path(genome), val(organism)
    output:
        tuple path("*.fa"), path("*.bed"), val(organism)
    script:
        """
        python ${params.scripts}/data_processing/segment_genome_noends.py ${genome}
        """
}
...
process generate_labels {
    ...
    publishDir "${params.publish_dir}/Single-NT_Preprocess/${organism}", mode: 'copy' 
    input:
        tuple path(fasta), path(bed), val(organism)
        path(crms)
    output:
        path("labels_matrix.dat")
    script:
        """
        python ${params.scripts}/data_processing/generate_single_nt_labels.py ${bed} ${crms}
        """
}
